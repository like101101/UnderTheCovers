...
checksum: ARGS ?= find / -not \( -path /proc -prune \) -not \( -path /sys -prune \) -not \( -path /etc -prune \) -type f 2>/dev/null -exec stat -c '%n %a' {} + # | sha256sum
checksum: DARGS ?= -u 0
...
shatest: ARGS ?= find /var/ -type f 2>/dev/null -exec stat -c '%n %a' {} + # | sha256sum
shatest: DARGS ?= -u 0
