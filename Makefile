...
checksum: ARGS ?= find / -not \( -path /proc -prune \) -not \( -path /sys -prune \) -not \( -path /var/logs -prune \) -type f -exec stat -c '%n %a' {} + | sort -k 1 | sha256sum
checksum: DARGS ?= -u 0
...
shatest: ARGS ?= find /etc -type f -exec stat -c '%n %a' {} + | sort -k 1 | sha256sum
shatest: DARGS ?= -u 0
